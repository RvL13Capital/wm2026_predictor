#!/usr/bin/env python3
"""
2018 World Cup backtest — model tips vs the actual results, from pre-2018 (June 2018) Elo,
...
(4/3/2) against the draw-heavy most-likely-score prediction. Output: validation/backtest_2018.txt
"""
import os, sys, csv
...
def main():
    t2g = {t: gg for gg, ts in w14.WM2018_GROUPS.items() for t in ts}
    rows = list(csv.DictReader(open("data/wc2018_results.csv")))
    out = []
    def emit(s=""):
        out.append(s); print(s)

    emit("=" * 72)
    emit("WORLD CUP 2018 BACKTEST — EV-optimal tip vs ACTUAL  (pre-2018 Elo, no lookahead)")
    emit("=" * 72)
    ...
    emit("Outcome accuracy  : see the prediction-vs-actual scorecard in report/wm2018_bracket.html")
    emit("=" * 72)

    os.makedirs("validation", exist_ok=True)
    with open("validation/backtest_2018.txt", "w") as f:
        f.write("\n".join(out) + "\n")
    print("\n💾 wrote validation/backtest_2018.txt", file=sys.stderr)
