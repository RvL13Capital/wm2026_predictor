#!/usr/bin/env python3
"""
2022 World Cup backtest — model tips vs the actual results, from pre-2022 (June 2022) Elo,
...
(4/3/2) against the draw-heavy most-likely-score prediction. Output: validation/backtest_2022.txt
"""
import os, sys, csv
...
def main():
    t2g = {t: gg for gg, ts in w14.WM2022_GROUPS.items() for t in ts}
    rows = list(csv.DictReader(open("data/wc2022_results.csv")))
    out = []
    def emit(s=""):
        out.append(s); print(s)

    emit("=" * 72)
    emit("WORLD CUP 2022 BACKTEST — EV-optimal tip vs ACTUAL  (pre-2022 Elo, no lookahead)")
    emit("=" * 72)
    ...
    emit("Outcome accuracy  : see the prediction-vs-actual scorecard in report/wm2022_bracket.html")
    emit("=" * 72)

    os.makedirs("validation", exist_ok=True)
    with open("validation/backtest_2022.txt", "w") as f:
        f.write("\n".join(out) + "\n")
    print("\n💾 wrote validation/backtest_2022.txt", file=sys.stderr)
