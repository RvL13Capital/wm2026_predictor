...
**Pool rules (verified — gate G1, `validation/POOL_RULES.md`):** 4/3/2 scoring where the goal-difference (3) tier **EXCLUDES draws** — a correct-but-inexact draw tip (e.g. 1:1 on a 0:0) scores only the tendency **2**, not 3 (confirmed with the pool owner 2026-06-24; see `get_points`). KO games are scored **"inkl. Elfmeterschießen"** → engine convention `kicktipp_ko_convention = "shootout_total"` (no draws; 90' + ET + every converted shootout kick summed into the final score).
...
- **λ calibration (`validation/points_recalibration.md`):** production `elo_baseline_goals=1.0` scores **289/192** under the corrected draws-excluded rule (was 299 under the old draw=3 assumption — the rule fix removed ~10 inexact-draw-on-draw points). **λ tuning is frozen until post-tournament**; `tests/test_lambda_points_floor.py` enforces ≥285 (re-baselined 2026-06-24). ⚠️ The low-λ→`0:0` rationale is now WEAKER (a wrong-score draw banks 2, not 3) — **λ should be re-optimized against the corrected objective post-tournament**. Points and calibration are different objectives (tipping optimizes points; the betting track needs calibration).
